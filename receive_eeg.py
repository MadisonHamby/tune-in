...
# Audio Variables, can change for diff waves
alpha_sound_threshold = 0.6
sound_file = "bell.mp3"  # sound that will be played when we meet alpha threshold
...
def abs_handler(address: str, *args):
    ...
    if (hsi[0] == 1 or hsi[1] == 1 or hsi[2] == 1 or hsi[3] == 1):
        ...
        rel_waves[wave] = math.pow(10, abs_waves[wave]) / (
                    math.pow(10, abs_waves[0]) + math.pow(10, abs_waves[1]) + math.pow(10, abs_waves[2]) + math.pow(10,
                                                                                                                    abs_waves[
                                                                                                                        3]) + math.pow(
                10, abs_waves[4]))
...
def plot_update(i):
    ...
    for wave in [0, 1, 2, 3, 4]:
        ...
        if (wave == 4):
            colorStr = 'orange'
            waveLabel = 'Gamma'
        plt.plot(range(len(plot_data[wave])), plot_data[wave], color=colorStr,
                 label=waveLabel + " {:.4f}".format(plot_data[wave][len(plot_data[wave]) - 1]))

    plt.plot([0, len(plot_data[0])], [alpha_sound_threshold, alpha_sound_threshold], color='black',
             label='Alpha Sound Threshold', linestyle='dashed')
    plt.ylim([0, 1])
    plt.xticks([])
...
if __name__ == "__main__":
    # Thread for plot render - Note this generates a warning, but works fine
    thread = threading.Thread(target=init_plot)
    thread.daemon = True  # allows main program to exit
    thread.start()
